...
2.x Source: https://gist.github.com/psa-jforestier/28a5de5d262ac2af5b00595e651696d0
...
parser.add_argument('-p',metavar='PASSWORD', dest='github_pass', action='store', required=False,
    ...
    default=None)
parser.add_argument('-t',metavar='TOKEN', dest='github_token', action='store', required=False,
    help='GitHub token',
...
if (args.github_token == None):
	...
	# We have a token - use args token if passed
	# base64string = base64.b64encode(args.github_token)
	headers = {"Authorization": "Bearer %s" % args.github_token, "Accept": "application/vnd.github.mercy-preview+json"}
...
while True:
	...
	if 'next' in list(res.links.keys()):
...
while True:
	...
	for repo in rep:
		...
		if (args.repo == None or reponame == args.repo):
			...
			for login,nbcom in sorted(iter(nbcommit_by_login.items()), key=lambda k_v: (k_v[1],k_v[0]), reverse=True):
